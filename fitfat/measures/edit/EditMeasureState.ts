...
import { findLastMeasure, findMeasure, storeMeasure } from '../model/MeasuresRepository'
...
export function useEditMeasureState(measureId: string | undefined) {
  ...
  const updateMeasureValueForMethod = (measureValue: MeasureValue, newValue: number) => {
    setState((s) => {
...
function fillValues(data: MeasuresData, measuresValues: MeasureValue[]): MeasureValueData[] {
