...
export const loadPreferences = () => {
    return async (dispatch: (action: PayloadAction<PreferencesState>) => void, getState: () => void) => {
        const preferences = await findPreferences()

        if (preferences) {
            dispatch(preferencesSlice.actions.update({
                isLoading: false,
...
export const savePreferences = () => {
    return async (dispatch: (action: PayloadAction<PreferencesState>) => void, getState: () => { preferencesReducer: PreferencesState }) => {
...
export const { updateDisplayName, updateBirthDate, updateHeight, updateWeight, updateSex, updateMeasureSystem } = preferencesSlice.actions
